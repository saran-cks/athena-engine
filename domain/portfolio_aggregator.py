...
class PortfolioAggregator:
    ...
    def aggregate(self, portfolio: Portfolio) -> List[CashFlow]:
        ...
        for tx in portfolio.transactions:
            if tx.amount <= 0:
                continue
            daily_investments[tx.date] = daily_investments.get(tx.date, 0.0) + tx.amount

        cashflows = [
            CashFlow(date=d, amount=-amt) for d, amt in daily_investments.items()
        ]
        cashflows.sort(key=lambda cf: cf.date)
        return cashflows

    def group_by_instrument(self, portfolio: Portfolio) -> Dict[str, List[Transaction]]:
        """
