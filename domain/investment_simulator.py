...
class InvestmentSimulator:
    ...
    def simulate(
        ...
        latest_nav_date = self.nav_service.get_latest_nav_date(target_instrument.scheme_code)
        if not latest_nav_date:
            raise ValueError(f"No NAV data available for '{target_instrument.name}' ({target_instrument.scheme_code}). Ensure data is synced.")
        ...
        nav_history = self.nav_service.get_nav_range(target_instrument.scheme_code, sorted_txs[0].date, end_date)
