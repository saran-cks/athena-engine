...
class AmfiClient:
    ...
    def fetch_daily_all(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            valid_lines = [line for line in lines if ';' in line and "Scheme Name" not in line]
            clean_data = "\n".join(valid_lines)
            
            # Parsing: Scheme Code;ISIN Div;ISIN Growth;Scheme Name;Net Asset Value;Date
            df = pd.read_csv(io.StringIO(clean_data), sep=';', header=None, on_bad_lines='skip')
            df.columns = ['scheme_code', 'isin_div', 'isin_reinv', 'scheme_name', 'nav', 'date']
